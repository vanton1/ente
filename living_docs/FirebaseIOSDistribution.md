...
| 3 | 3.1 | Publish the audited iOS baseline through Firebase | M | 🔴 blocked / needs decision | Museum/storage health, signing, generation, archive/export, audit, immutable `1.3.59` (`2160`) pair, preflight, exact confirmation, and one Firebase distribute call completed. The CLI returned exit `0` JSON success without receipt references; the guard preserved a mode-`0444` attempt instead of claiming success. Official read-only evidence proves exactly one matching commit-bound release with all references, and Task 2.8's real no-upload preflight now passes against it. Resume only after Task 2.8 is committed and pushed, then run reconciliation once to create the success receipt—never upload again. No Apple or installed-app state changed. |
