...
**Status:** V1 complete as of 2026-07-19. V1.1 Tasks 3.6–3.8 are deferred because no real second iPhone or iPad is available.
**Started:** 2026-07-17
**Owner:** vanton
**Planning doc:** n/a
**Companion docs:** `mobile/apps/photos/SELF_HOSTED_DOCUMENTATION.md`, `living_docs/ConfigurableSelfHostedMobileServer.md`, `living_docs/ConfigurableSelfHostedMobileServerArchitecture.md`, `living_docs/LockedSelfHostedIOS.md`, `living_docs/FirebaseAndroidDistribution.md`, `mobile/apps/photos/SELF_HOSTED_BUILD_GUIDE.md`, `mobile/apps/photos/SELF_HOSTED_IOS_DISTRIBUTION_GUIDE.md`, `living_docs/FirebaseIOSDistributionArchitecture.md`
...
| 1 | 1.5 | Create the owner-only Ad Hoc provisioning profile | S | 🟢 done | Generated and downloaded the manual owner-only Ad Hoc profile, expiring 2027-07-17. Local decoding verified the exact bundle/application-identifier binding, Cytech team binding without printing its identifier, one authorized device, one distribution certificate, and non-debug Ad Hoc state. The embedded certificate fingerprint matches Task 1.3. The profile UUID, `.mobileprovision` file, Team ID, and device identifier remain outside Git. |
...
| 2 | 2.8 | Reconcile Firebase JSON-only publication success | M | 🟢 done | Publisher `1.1.0` adds a no-upload reconciliation mode over the immutable partial attempt and private mode-`0444` official release-list response. It re-audits the IPA, rechecks the active app/group and ledger, requires exit-`0` JSON success plus exactly one matching app resource, version/build, notes, creation window, and three known-host references, supports a no-write preflight, preserves both inputs, and collision-safely writes the normal success receipt with reconciliation hashes/provenance. Future distribute calls request reference-bearing CLI output instead of JSON-only mode. Formatting, analysis, Bash/help, privacy/diff checks, all 21 publisher and 38 combined tests pass. A real build-`2160` preflight matched the official release and wrote no receipt or Firebase mutation. |
| 3 | 3.1 | Publish the audited iOS baseline through Firebase | M | 🟢 done | Published the immutable `1.3.59` (`2160`) IPA/manifest pair after Museum/storage health, signing, clean-checkout generation, archive/export, audit, preflight, and exact confirmation passed. The first CLI response returned exit `0` JSON success without receipt references, so the guard preserved a mode-`0444` attempt and no retry occurred. After Task 2.8 was committed and pushed, one no-upload reconciliation matched the immutable attempt to the official read-only release-list evidence and wrote the unique mode-`0444` success receipt. An independent receipt check verified the exact build, both evidence hashes, official API method, preserved attempt, and `noUploadPerformed: true`. |
| 3 | 3.2 | Verify the owner's Firebase installation | S | 🟢 done | Accepted the Firebase invitation and installed its device-registration profile plus build `2160` on the owner's iPhone. After replacing one failed downloaded registration profile with a fresh profile, the distributed app launched, retained the configurable local server, authenticated to the intended Museum account, synced an iPhone upload to the local Photos web app, downloaded and decrypted a separate web upload, and preserved the login, server, library, and readable media after a forced process restart. No legacy-app removal was performed. |
...
| 3 | 3.6 | Register one non-owner tester device | S | 🔴 blocked / needs decision | The intended tester joined `trusted-ios-testers`, accepted the invitation, and produced one privately exported registration identifier that was added to Apple's inventory. The owner later clarified that no real second iPhone or iPad exists, so the identifier cannot be accepted as evidence of a non-owner iOS device and must not enter a provisioning profile. Firebase-group and Apple-inventory cleanup require separate explicit approval; no identity or identifier entered Git. |
| 3 | 3.7 | Refresh provisioning and publish a tester-compatible build | M | ⚪ not started | Deferred because Task 3.6 has no verified second iPhone or iPad. The downloaded profile was rejected before private storage, installation, signing, preparation, or publication; the owner-only profile and Firebase build `2161` remain authoritative. Resume only after a real non-owner iOS device completes registration. |
...
Task 1.5 generated the manual owner-only Ad Hoc profile for the explicit self-hosted App ID, the separately owned distribution certificate, and exactly one owner device. Local decoding verified a non-debug profile, exact application/team binding, one device, one certificate, a privately retained UUID, and expiry on 2027-07-17. The embedded certificate has the same public SHA-256 fingerprint recorded in Task 1.3. The profile remains a local external signing input and is not committed because it embeds the private device list and team identifier.
...
Task 2.1 registered `me.vanton.ente.photos.selfhosted` as the sole iOS application in the existing self-hosted Firebase project, with display name `Ente Photos Self-Hosted iOS`. A post-creation inventory independently matched the platform, display name, bundle, and returned Firebase App ID. The publication binding lives only in `<private-ios-toolchain-root>/firebase/distribution.env`; its parent directory is mode `700`, the file is mode `600`, and the App ID is absent from Git. No `GoogleService-Info.plist` or runtime configuration was generated because later publication commands need only the local project/App-ID binding.
...
Task 2.8 added a no-upload recovery path for the real case where Firebase CLI returned exit `0` and JSON success after creating the release but omitted every reference required by the receipt contract. Reconciliation consumes the preserved immutable attempt plus an immutable response from Firebase's official read-only release-list API, then repeats the IPA, app, group, ledger, notes, version/build, creation-window, and reference checks before writing the ordinary success receipt. It hashes and preserves both evidence inputs and records `noUploadPerformed: true`; it cannot invoke the distribute operation. Future publication calls use Firebase's reference-bearing text output rather than JSON-only status.

Task 3.1 prepared and published `1.3.59` (`2160`) from pushed source. The ambiguous initial response was never retried: after Task 2.8 reached the public fork, one real reconciliation bound the existing Firebase release to its immutable artifact, manifest, partial attempt, official API evidence, and unique read-only success receipt. Task 3.2 then proved the owner-facing channel on a physical iPhone. Firebase profile registration and installation delivered the audited bundle, and the installed app authenticated to the intended private service, completed uploads in both directions between iPhone and local web app, decrypted the remotely introduced media, and retained account, server, and library state across a forced restart.

Task 3.3 closed the side-by-side rollback period only after the owner confirmed possession of the local account password and recovery key and the replacement's cloud copy had been proven from both the iPhone and local web app. The private device inventory found the legacy bundle already absent and only the verified Firebase build present, so retirement required no uninstall mutation and no app-local state migration.

Task 3.4 proved the forward-update publication path with `1.3.59` (`2161`). Local and private Museum/storage checks returned healthy before preparation. The first two isolated attempts failed before artifact creation because the operator command did not carry the already documented pinned Flutter/Dart binaries and rustup-first PATH; restoring the complete proven environment allowed clean dependency resolution, source generation, archive/export, and audit from pushed commit `d61b831c33`. The immutable artifact retained the bundle, configurable server policy, one-device Ad Hoc scope, distribution certificate, arm64-only core target, and zero extensions. A non-mutating Firebase preflight passed, one release-specific confirmation authorized one upload, and the normal read-only success receipt bound the created release and all three Firebase references to the exact manifest and IPA hashes without a partial attempt.

Task 3.5 installed that Firebase release over the existing owner baseline. Private device inventories established build `2160` immediately before installation and build `2161` afterward under the same bundle identity. The application opened without reauthentication or server reconfiguration, retained the existing cloud library, completed a fresh controlled encrypted media round trip in both directions between the iPhone and local web app, and preserved the account, server binding, library, and readable media after a forced process restart. This proves the guarded higher-build publication is an in-place update rather than a clean installation.
...
| Item | Status | Why |
|---|---|---|
| Register, provision, publish for, and verify one non-owner iOS device (Tasks 3.6–3.8) | V1.1 backlog | No real second iPhone or iPad is available. Resume only when a non-owner device can complete Firebase registration and physical acceptance; do not use the current unverified identifier or downloaded profile. |
...
| Change the Android Firebase tracker | Out of scope | Android device and non-owner distribution acceptance were completed in the separate Android initiative; this iOS record does not own that tracker. |
...
### 2026-07-20 — Pause non-owner iOS distribution after withdrawing the device premise

**Decision:** Return Task 3.7 to deferred work and mark Task 3.6 blocked after the owner clarified that no real second iPhone or iPad exists. Do not preserve, install, sign with, or publish the newly downloaded profile, and do not treat the privately exported identifier as non-owner iOS acceptance evidence. Keep the proven owner-only build and profile authoritative.

**Why:** Ad Hoc authorization and physical acceptance require a real registered iPhone or iPad. A MacBook is a macOS device and cannot establish the required iOS installation evidence, so continuing with a two-device profile would encode a false premise and publish an untestable artifact.

**Alternatives considered:** Continue based only on the exported identifier, which cannot prove a real tester device; use the MacBook as the second device, which does not test the iOS Ad Hoc application; or remove Firebase and Apple state immediately, which is a separate external mutation that the owner has not yet approved.

### 2026-07-19 — Publish the first two-device iOS build

**Decision:** Move Task 3.7 into active V1.1 work. Create a new Ad Hoc profile containing exactly the owner and approved tester iPhones, require build `2162`, and use the existing immutable preparation, audit, confirmation, publication, and receipt workflow. Keep physical tester acceptance in Task 3.8.

**Why:** Task 3.6 established both Firebase membership and Apple authorization for exactly one non-owner device. A new profile and higher immutable build are now required because the published owner-only IPA cannot install on that iPhone, even though Firebase can offer it to the tester.

**Alternatives considered:** Reuse build `2161`, which would map two different signed IPAs to one build identity; expand the profile with additional devices, which would exceed the approved tester scope; or combine publication and tester acceptance, which would erase the review boundary between release creation and device behavior.

### 2026-07-19 — Complete non-owner device authorization before reprovisioning

**Decision:** Close Task 3.6 after one approved tester joined the dedicated iOS group, registered one iPhone through Firebase, and had that exact privately exported device enabled in Apple Developer. Leave the existing owner-only profile, published build, and tester installation untouched until Task 3.7.

**Why:** Comparing Firebase's project-level device export with the known owner-only profile isolated exactly one new device without copying identity or UDID evidence into Git. Apple registration establishes the device authorization dependency while preserving the review boundary before profile expansion and publication.

**Alternatives considered:** Manually copy a displayed UDID, which increases transcription and disclosure risk; regenerate the profile and publish within Task 3.6, which would collapse separate authorization and release mutations; or treat Firebase registration alone as sufficient, which would still leave the Ad Hoc device unauthorized by Apple.

### 2026-07-19 — Resume non-owner iOS device registration

**Decision:** Move Task 3.6 from the V1.1 backlog into active work and register one real non-owner tester's iPhone through the existing `trusted-ios-testers` Firebase flow and Cytech Apple device inventory. Keep profile expansion, publication, and installation acceptance in Tasks 3.7 and 3.8.

**Why:** The owner explicitly resumed Task 3.6 after completing the Android distribution and reconciliation work. Registering the real device is the smallest pause-safe step: it proves the Firebase-to-Apple handoff without yet rebuilding, publishing, or notifying the wider group about a new release.

**Alternatives considered:** Leave all non-owner iOS work deferred; combine registration, profile refresh, publication, and acceptance into one large task; or automate signing/distribution first. Deferral would not extend the proven channel, bundling would erase review boundaries, and automation would introduce credentials before the manual additional-device workflow is proven.
...
**Decision:** Preserve the immutable partial-attempt record, capture a private immutable response from Firebase's official read-only release-list API, and add a guarded reconciliation mode that re-audits all local and Firebase bindings before writing the normal success receipt without an upload. Make future distribute calls request Firebase's reference-bearing human-readable output instead of JSON-only status.

**Why:** The installed Firebase CLI returned exit `0` and `{"status":"success"}` after creating and distributing build `2160`, but omitted the console, tester, and binary references required by the receipt contract. Official API evidence supplies those references and independently proves the exact app/version/build/source-notes release. Binding both immutable records retains fail-closed provenance while preventing an unsafe duplicate upload.

**Alternatives considered:** Retry the IPA, which could duplicate or re-notify an already successful release; trust status alone or fabricate URLs, which would weaken the receipt; or accept a console screenshot/manual note, which is mutable and cannot be machine-bound to the prepared artifact. None preserves the existing audit and increasing-build ledger as strongly as official immutable evidence plus no-upload reconciliation.
...
**Decision:** Use `<private-ios-toolchain-root>` outside Git, with `signing` for profiles, `baseline-artifacts` for Phase 1 archives/IPAs/logs, and later `prepared-releases` and `firebase-receipts` directories for guarded publication evidence.
...
- Export Firebase's project-level iOS tester-device inventory privately and compare it against the current profile rather than copying a UDID by hand. Requiring exactly one difference made the Firebase-to-Apple handoff both less error-prone and independently bounded.
- Confirm that an additional identifier belongs to a real, available non-owner iPhone or iPad before registering it with Apple or starting reprovisioning. A syntactically valid Firebase export is not physical-device acceptance evidence.
