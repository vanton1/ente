...
| 2 | 2.2 | Add confirmed push, pull-request creation, and issue handoff | M | 🟡 working | Require a clean validated branch and exact typed confirmation before pushing only to `origin`; create one PR into fork `main`, link the current upstream-drift issue, and never merge the PR automatically. |
