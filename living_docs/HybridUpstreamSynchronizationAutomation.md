...
| 1 | 1.2 | Add guarded integration-branch creation and exact-SHA merge | M | 🟡 working | Create a dated collision-safe sync branch from the accepted fork `main`, merge only the recorded official SHA with full history, and stop with preserved evidence and explicit resume/abort guidance on conflicts or unexpected state. |
| 2 | 2.1 | Add dependency, test, analysis, and optional platform-build gates | M | ⚪ not started | Run deterministic submodule/dependency/generation checks, the focused self-hosted suites, tracked-source formatting, full mobile analysis, and opt-in Android debug/iOS Simulator builds against a public example endpoint. Stop on the first failure and never reinterpret failure as success. |
| 2 | 2.2 | Add confirmed push, pull-request creation, and issue handoff | M | ⚪ not started | Require a clean validated branch and exact typed confirmation before pushing only to `origin`; create one PR into fork `main`, link the current upstream-drift issue, and never merge the PR automatically. |
| 3 | 3.1 | Add scheduled GitHub drift detection with one tracking issue | M | ⚪ not started | Add a daily and manually runnable fork-only workflow with read-only contents and issue-write permissions. It opens or updates one marker-based issue when official Ente is ahead and closes the issue when no upstream-only commits remain. |
| 3 | 3.2 | Test safe runs, failures, issue idempotency, and recovery | M | ⚪ not started | Cover no-change, dirty-tree, unsafe-remote, missing-tool, collision, merge-conflict, generation drift, failed validation, confirmation, push/PR, workflow permissions, duplicate-issue prevention, and safe resume behavior without network or external mutation. |
| 3 | 3.3 | Document the one-command operator and recovery workflow | S | ⚪ not started | Add current commands, prerequisites, expected issue/PR lifecycle, optional-build behavior, failure recovery, and strict release boundaries to the canonical maintenance documentation. |
| 3 | 3.4 | Update the as-built synchronization architecture | S | ⚪ not started | Update the existing architecture companion with the implemented local command, GitHub detector, state transitions, permission model, rollback paths, and first automation validation evidence. |
