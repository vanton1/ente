...
**Status:** V1 and V1.1 complete as of 2026-07-19. The guarded Android release ledger now contains the reconciled build-`2158` and build-`2159` Firebase releases.
**Started:** 2026-07-16
**Owner:** vanton
**Planning doc:** n/a
**Companion docs:** `mobile/apps/photos/SELF_HOSTED_DOCUMENTATION.md`, `living_docs/ConfigurableSelfHostedMobileServer.md`, `living_docs/ConfigurableSelfHostedMobileServerArchitecture.md`, `living_docs/LockedSelfHostedAndroid.md`, `mobile/apps/photos/SELF_HOSTED_BUILD_GUIDE.md`, `mobile/apps/photos/SELF_HOSTED_ANDROID_DISTRIBUTION_GUIDE.md`, `mobile/apps/photos/README.md`
...
| 1 | 1.2 | Build, sign, and audit the replacement APK | S | 🟢 done | Built the configurable release for the private Museum origin and audited the fresh APK as package `me.vanton.ente.photos.selfhosted`, version `1.3.59` (`2158`), min/target SDK `26`/`36`, non-debuggable, and limited to `arm64-v8a` plus `armeabi-v7a`. APK Signature Scheme v2 verifies with the expected certificate SHA-256 `9f0a5f39668e7098d097745931bcb8fc392d50da877cf349a2b20e2db1a4ce69`; archive integrity passes and APK SHA-256 is `57d90841070903430374bb4dda3339b737a4980cfafa9659f73e6e2a235c50ae`. Preserved the exact 262,750,609-byte artifact in `<private-toolchain-root>/artifacts` without overwriting the legacy-package artifact or changing installed application state. |
| 2 | 2.1 | Provision Firebase and the trusted tester group | S | 🟢 done | Installed Firebase CLI `15.24.0` in the external Android toolchain and authenticated it as the intended operator with Gemini and optional telemetry disabled. Created one dedicated private Firebase project, registered one active Android app with exact audited package `me.vanton.ente.photos.selfhosted`, initialized App Distribution and confirmed its contact email in the console, and verified group alias `trusted-testers`. Added no Android Firebase configuration, runtime integration, credential, tester identity, or project binding to Git. During verification, Firebase CLI `login:list --json` unexpectedly printed OAuth tokens; the session was immediately revoked and replaced through a fresh browser login, which was safely verified through project visibility. |
...
| 4 | 4.1 | Add no-upload Android release reconciliation | M | 🟢 done | Publisher `1.1.0` adds a no-upload mode over one immutable partial attempt and immutable official Firebase release-list response. It re-audits the APK, rechecks the active app/group and increasing-version ledger, requires exit-`0` JSON success plus one exact app resource/version/build/notes/time/known-host-reference match, supports a no-write preflight, preserves both inputs, and writes the normal mode-`0444` receipt with evidence hashes and `noUploadPerformed: true`; future uploads request reference-bearing text. Formatting, analysis, Bash/help, privacy/diff checks, 19 focused publisher tests, and 31 combined preparation/publication tests pass. Commit `8fd969592f` is pushed. One owner-only mode-`0444` official response containing exactly builds `2158` and `2159` passed both live no-write preflights; ascending no-upload reconciliation then produced and independently validated exactly two mode-`0444` success receipts while preserving both attempts. |
...
| Distribute the iOS application through Firebase | Completed separately | The follow-on iOS initiative published owner builds `2160` and `2161` and verified owner installation and updates; non-owner iOS acceptance remains deferred. See `living_docs/FirebaseIOSDistribution.md`. |
...
**Status values:**
- `Completed separately` — delivered by the linked follow-on initiative rather
  than by this Android tracker.
...
**Decision:** Use one dedicated private Firebase project and Android App ID, exact package `me.vanton.ente.photos.selfhosted`, and stable tester-group alias `trusted-testers` for this closed beta. Keep the project/App-ID binding in private operator configuration rather than this public record.
