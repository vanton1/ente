...
|     1 |  1.3 | Establish a least-privilege App Store Connect publication credential                |  S   | ⚪ not started | At a separate approval boundary, choose an app-limited individual key or explicitly accept a team-wide key, establish only the required App Manager access, store the private key outside Git with owner-only permissions, and prove authentication through read-only requests before authorizing publisher tooling.                                                                                                                                                                                                                                                                                                                                  |
|     1 |  1.4 | Add an App Store distribution export mode while retaining Ad Hoc fallback           |  M   | ⚪ not started | Extend the guarded iOS builder for App Store signing and export. Keep the proven Ad Hoc path intact until external TestFlight acceptance completes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|     1 |  1.5 | Prepare and audit an immutable TestFlight IPA and manifest                          |  M   | ⚪ not started | Reuse the isolated pushed-source build model, but bind the output to App Store signing, the configurable endpoint policy, the review Museum origin, and TestFlight-specific identity checks without contacting Apple services.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- Can a dedicated App Manager user be limited to the Ente app and issue an
  individual App Store Connect API key, or must V1 explicitly accept a team key
  that can reach every Cytech app?
